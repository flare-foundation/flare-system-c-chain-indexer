...
- **`log_range`** — max blocks per `eth_getLogs` request. **Set this to your RPC node's getLogs limit.** Many providers cap the block range (commonly 1000–10000) or the number of returned results. The indexer checks this at startup with one `eth_getLogs` over exactly `log_range` blocks and refuses to start if the node rejects it, naming the limit to lower. Use a conservative value on shared/public endpoints and a larger one on your own node to reduce the number of log requests. This is the only knob you usually need to know your node for.
- **`rpc_concurrency`** — max simultaneous RPC calls of every kind, enforced process-wide: block, receipt and log (`eth_getLogs`) fetches share this single budget, as do contract calls and history-drop lookups. This is the main throughput dial, since block fetching dominates catchup. Raise it to speed up catchup against a dedicated or underutilized node; lower it if a shared or rate-limited endpoint returns 429s or times out — note that lowering it below the number of log filters (logged at startup as `Indexer collection policy: … log filters`) makes continuous indexing fetch a block's filters in several rounds instead of one. The default of 25 is sized for a shared endpoint and covers the built-in FSP filters; leave it otherwise.
...
Within a batch, block fetching and log fetching run concurrently (they have no data dependency, though they share the `rpc_concurrency` budget), and the indexer issues one `eth_getLogs` per configured log filter, tiled into `log_range`-sized chunks when `batch_size` exceeds `log_range`. The filters are fetched concurrently, so a batch — and, in continuous indexing, a single block — costs about `ceil(filters / rpc_concurrency)` rounds of log requests rather than one per filter.
