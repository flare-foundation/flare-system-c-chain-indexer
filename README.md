...
- **`rpc_concurrency`** — max simultaneous point-RPC calls (block and receipt fetches); it does not affect log fetching. This is the main throughput dial, since block fetching dominates catchup. Raise it to speed up catchup against a dedicated or underutilized node; lower it if a shared or rate-limited endpoint returns 429s or times out. Leave the default otherwise.
...
Within a batch, block fetching and log fetching run concurrently (they are independent RPC streams), and the indexer issues one `eth_getLogs` per configured log filter, tiled into `log_range`-sized chunks when `batch_size` exceeds `log_range`.
